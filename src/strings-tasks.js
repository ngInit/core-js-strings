...
function countVowels(str) {
  const matcher = 'aeiouyAEIOUY';
  let result = 0;
  for (let i = 0; i < str.length; i += 1) {
    result += matcher.includes(str.charAt(i)) ? 1 : 0;
  }
  return result;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrayOfWords.length; i += 1) {
    longestWord =
      arrayOfWords[i].length > longestWord.length
        ? arrayOfWords[i]
        : longestWord;
  }
  return longestWord;
...
function encodeToRot13(str) {
  ...
  let encryptedString = '';
  for (let i = 0; i < str.length; i += 1) {
    const indexOfSymbol = origAlphabet.indexOf(str.charAt(i));
    const newSymbolByIndex = secretAlphabet.charAt(indexOfSymbol);
    encryptedString += str[i].match(/\W/g) ? str[i] : newSymbolByIndex;
  }
...
function getCardId(value) {
  ...
  const suits = '♣♦♥♠';
  const card = rs.indexOf(value.slice(0, value.length - 1));
  const multiplier = suits.indexOf(value.slice(value.length - 1)) * 13;
